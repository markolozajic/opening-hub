...
- **Preparation (player filter)**: `PreparationRecord` stored in Dexie `preparation` table keyed by `[repertoire+player]`, each containing `taggedFens: string[]`. `tagPosition()` tags a FEN for a player along with all non-`everybody` ancestors on `nav.currentPath`. `untagPosition()` cascades to all descendants. `getOpponentsAt()` returns `{ name, certain }[]` — `certain: true` for direct tags or positions on the path to a tagged position (fen is ancestor of a taggedFen); `certain: false` for positions reachable without opponent deviation at any turn-role position along the path; not shown if the opponent deviated (opponent has tagged children at a turn-role position but the actual move isn't one of them). The path is built from `nav.currentPath` (the user's actual navigation) rather than `getAncestors` to avoid transposition false positives. All players taggable at any position. "Played by: everybody" hardcoded at root and after first player move, controlled by `isEverybodyPly()`. `nextSansForPath()` handles player filter narrowing — null = show all, [] = outside repertoire, SANs = show only those.
