...
- **Preparation (player filter)**: `PreparationRecord` stored in Dexie `preparation` table keyed by `[repertoire+player]`, each containing `taggedFens: string[]`. `tagPosition()` tags a FEN for a player along with all non-`everybody` ancestors on `nav.currentPath`. `untagPosition()` cascades to all descendants. `getPlayersAt()` returns `{ name, certain }[]` — `certain: true` for direct tags or positions on the path to a tagged position (fen is ancestor of a taggedFen); `certain: false` for positions reachable without player deviation at any player-turn position along the path; not shown if the player deviated (has tagged children at a player-turn position but the actual move isn't one of them). All players taggable at any position. "Played by: everybody" hardcoded at root and after first White move, controlled by `isEverybodyPly()`. `nextSansForPath()` handles player filter narrowing — null = show all, [] = outside repertoire, SANs = show only those.
